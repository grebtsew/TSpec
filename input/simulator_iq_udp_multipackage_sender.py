...
def generate_metadata(stream_id, center_freq, sample_rate, bandwidth, lat, lon, total_packets):
    return {
        "stream_id": stream_id,
...
def send_data(iq_data, metadata, ip="127.0.0.1", port=5005):
    ...
    for i in range(0, len(iq_bytes), DATA_CHUNK_SIZE):
        ...
        header = stream_id[:16].ljust(16, b' ') + (i // DATA_CHUNK_SIZE).to_bytes(4, 'big')
...
def main():
    ...
    while True:
        ...
        metadata = generate_metadata(stream_id, center_freq, sample_rate, bandwidth, lat, lon, total_packets)
