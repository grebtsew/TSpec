...
parser = argparse.ArgumentParser(description="Simulator som skickar raw IQ-float32 via UDP")
parser.add_argument("--host", type=str, default="127.0.0.1",
                    help="Mottagarens IP")
parser.add_argument("--port", type=int, default=5005,
                    help="UDP-port")
parser.add_argument("--samplerate", type=float, default=1e6,
                    help="Samplerate i Hz")
parser.add_argument("--center-freq", type=float, default=0.0,
                    help="Center frequency [Hz]")
parser.add_argument("--bandwidth", type=float, default=200e3,
                    help="Bredd på simulerad signal [Hz]")
parser.add_argument("--blocksize", type=int, default=1024,
                    help="Antal sampel per UDP-paket")
parser.add_argument("--send-rate", type=float, default=0.01,
                    help="Tid mellan paket [s]")
...
# --- Huvudloop ---
def main():
    print(f"Skickar simulering till {args.host}:{args.port} "
          f"SR={args.samplerate} Hz, block={args.blocksize}")
    stream_id = send_metadata()
    
    try:
        while True:
            iq_block = generate_iq(args.blocksize, args.samplerate, bandwidth=args.bandwidth)
